...
export default function ItemsPage() {
  ...
  const { page, pageCount, pageSize, paginatedItems, setPage } = usePaginatedData(filteredItems);
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-white border border-[#d8dee6] rounded-md shadow-sm">
             ...
             <span className="text-[10px] font-bold text-[#263238] uppercase tracking-widest">Master Data Node</span>
          </div>
          <h1 className="text-3xl font-extrabold text-[#263238] tracking-tight uppercase italic">
            Component <span className="text-[#f59e0b]">Registry</span>
          </h1>
          <p className="text-[#64748b]/70 text-sm font-medium">Define your master item list and SKUs before procurement.</p>
        </div>
      ...
      <div className="space-y-3">
        <ListToolbar
          ...
          placeholder="Search master items by name, SKU, or category..."
      ...
      {/* ITEMS LIST matching Quotations table style */}
      <div className="bg-white rounded-md border border-[#d8dee6] shadow-[0_20px_50px_rgba(5,91,101,0.04)] overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[#f7f4ef]/50 border-b border-[#d8dee6]">
                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.25em] text-[#64748b]">SKU / ID</th>
                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.25em] text-[#64748b]">Item Definition</th>
                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.25em] text-[#64748b]">Category</th>
                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.25em] text-[#64748b]">Default Pricing</th>
            ...
            <tbody className="divide-y divide-[#d8dee6]/30">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-8 py-24 text-center">
                    <div className="flex flex-col items-center gap-4">
                       ...
                       <p className="text-[10px] font-bold text-[#64748b]/40 uppercase tracking-widest">Indexing Master Nodes...</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-24 text-center">
                    <p className="text-sm font-bold text-[#64748b]/40">No item records match your search.</p>
                  </td>
                </tr>
              ) : (
                paginatedItems.map((item) => (
                  <tr key={item._id} className="hover:bg-[#f7f4ef]/30 transition-colors group">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                         <div className="w-10 h-10 rounded-md bg-[#263238] flex items-center justify-center text-[#f59e0b] transition-all group-hover:scale-110">
                            <Database size={16} />
                         </div>
                         <div>
                            <p className="text-sm font-black uppercase tracking-tight text-[#263238]">{item.name}</p>
                            <p className="text-[9px] font-bold text-[#64748b]/40 uppercase tracking-widest mt-1">Master Ref: {item._id.slice(-6)}</p>
                         </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                      <div className="space-y-1">
                         <p className="text-sm font-black text-[#263238]">Rs.{Number(item.price || 0).toLocaleString()}</p>
                         <p className="text-[8px] font-bold text-[#64748b]/60 uppercase tracking-widest">Base Rate</p>
                      </div>
                    </td>
        ...
        <ListPagination page={page} pageCount={pageCount} pageSize={pageSize} total={filteredItems.length} onPageChange={setPage} />
      </div>
    </div>
  );
}
